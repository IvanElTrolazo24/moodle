...
class Usuario
{
	...
	{	
		...
		} elseif ($rst->num_rows == 1) { //mysql_result
			//usuario encontrado 
	...
	public function nuevo(){
		...
		$query = sprintf("insert into 
			usuarios(username,password,email,nombres,apellidos,foto,rol_id) 
			values('%s','%s','%s','%s','%s','%s','%d')"); 
	}
	public function buscarPorUsername(string $username){
		$cnn = new Conexion();
		$query = sprintf("select username from usuarios where username = %s",$usermame);
		$rst = $cnn->query($sql);
		$cnn->close();
		if (!$rst) {
	...
	public function buscarPorEmail(string $email){
		$cnn = new Conexion();
		$query = sprintf("select username from usuarios where email = %s",$email);
		$rst = $cnn->query($sql);
		$cnn->close();
		if (!$rst) {
			die('Error al ejecutar la consulta');
